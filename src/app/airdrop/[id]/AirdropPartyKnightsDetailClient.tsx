...
function PartyKnightsClaimPeriodBadge({ start, end }: { start?: string; end?: string }) {
  return (
    <div className="mt-4">
...
function PartyKnightsOverviewCard({
  isVestingLoading,
  claimableAmount,
  isClaiming,
  onClaim,
  ...
  isClaiming: boolean;
  onClaim: () => void;
  ranking: { rank: string; tier: string; score: string; loading: boolean };
}) {
  const isDisabled = isVestingLoading || isClaiming || !claimableAmount || new Big(claimableAmount).lte(0);
    ...
    <div className="bg-[#121212] rounded-lg border border-[#2D2D2D] mb-5 overflow-hidden">
      <div className="grid grid-cols-1 min-[800px]:grid-cols-3 border-b border-[#2D2D2D]">
        ...
        <div className="px-4 py-4 border-b border-[#2D2D2D] min-[800px]:border-b-0 min-[800px]:border-r">
          ...
          <div className="mt-1.5 text-white text-xl font-semibold leading-[1.2] tracking-[0]">
            {ranking.loading ? '...' : `${getTierEmoji(ranking.tier)} ${ranking.tier}`.trim()}
          </div>
        </div>
...
function getPartyKnightsHistoryStatusLabel(status?: string) {
...
function getTierEmoji(tier?: string) {
  const normalized = String(tier || '')
    .trim()
    .toLowerCase();
  if (normalized === 'platinum') return '🏆';
  if (normalized === 'gold') return '🥇';
  if (normalized === 'silver') return '🥈';
  if (normalized === 'bronze') return '🥉';
  return '';
}

export default function AirdropPartyKnightsDetailClient({ id }: { id: string }) {
  ...
  const [partyKnightsRanking, setPartyKnightsRanking] = useState<{
    rank: string;
    tier: string;
    score: string;
    loading: boolean;
  }>({ rank: '-', tier: '-', score: '-', loading: false });
  ...
  // Fetch reward + claimable from Party Knights reward contract
  const fetchReward = React.useCallback(async () => {
    ...
    try {
      ...
      console.error('Failed to fetch Party Knights reward:', error);
      setReward(null);
      setClaimableAmountRaw(null);
    } finally {
      setIsVestingLoading(false);
  ...
  // Fetch Party Knights rank/tier/score by wallet
  useEffect(() => {
    let cancelled = false;

    const fetchPartyKnightsRanking = async () => {
      ...
      if (!cancelled) {
        setPartyKnightsRanking((prev) => ({ ...prev, loading: true }));
      }

      try {
        const url = `${baseUrl}/api/hpp-user/rankings/by-wallet/${address}?month=${month}`;
        const resp = await axios.get(url, { headers: { accept: 'application/json' } });
        const ok = !!resp?.data?.ok;
        const first = resp?.data?.data?.items?.[0];

        if (!cancelled) {
          if (ok && first) {
            setPartyKnightsRanking({
              rank: String(first.rank ?? '-'),
              tier: String(first.tier ?? '-'),
              score: String(first.monthlyScore ?? '-'),
              loading: false,
            });
          } else {
            // Non-ambassador or no record for month
            setPartyKnightsRanking({ rank: '-', tier: '-', score: '-', loading: false });
          }
        }
      } catch {
        if (!cancelled) {
        ...
        >
          ...
          {isConnected && airdropDetail && (
            <div className="px-5 max-w-6xl mx-auto mt-7.5 mb-20">
              ...
              {/* Overview / Party Knights Card */}
              <PartyKnightsOverviewCard
                isVestingLoading={isVestingLoading}
                claimableAmount={claimableAmount}
                isClaiming={isClaiming}
                onClaim={() => void onClaimTokens()}
                ranking={partyKnightsRanking}
              />
                                    ...
                                    <div className="flex items-center gap-2 text-white text-sm leading-[1.2] tracking-[0.8px]">
                                      <span>
                                        {tx.status === 'Pending' ? (
                                          ...
                                          <span
                                            className={
                                              getPartyKnightsHistoryStatusLabel(tx.status) === 'Claimed'
                                                ? 'text-[#5DF23F]'
                                                : ''
                                            }
                                          >
                                            {getPartyKnightsHistoryStatusLabel(tx.status)}
                                          </span>
                                        )}
                                      </span>
                                      <a
