...
export default function MigrationClient({
  ...
  const [migrationHash, setMigrationHash] = useState<`0x${string}` | null>(
    null,
  );
  ...
  const updateHistoryWithServer = (incoming: Transaction[]) => {
    ...
    setTransactionHistory((prev) => {
      const pendingIds = new Set(
        prev.filter((t) => t.status === "Pending").map((t) => t.id),
      );
      const pendingResolved = sortedIncoming.some(
        (t) => pendingIds.has(t.id) && t.status !== "Pending",
      );
  ...
  const HPP_TOKEN_ADDRESS = process.env
    .NEXT_PUBLIC_ETH_HPP_TOKEN_CONTRACT as `0x${string}`;
  const ADDRESSES = {
    AERGO: {
      token: process.env.NEXT_PUBLIC_ETH_AERGO_TOKEN_CONTRACT as `0x${string}`,
      migration: process.env
        .NEXT_PUBLIC_ETH_AERGO_HPP_MIGRATION_CONTRACT as `0x${string}`,
    },
    AQT: {
      token: process.env.NEXT_PUBLIC_ETH_AQT_TOKEN_CONTRACT as `0x${string}`,
      migration: process.env
        .NEXT_PUBLIC_ETH_AQT_HPP_MIGRATION_CONTRACT as `0x${string}`,
    },
  } as const;
  ...
  const { isSuccess: isMigrationSuccess, isError: isMigrationError } =
    useWaitForTransactionReceipt({
  ...
  const startTransactionPolling = (txHash: string, walletAddress: string) => {
    ...
    const pollTransaction = async () => {
      try {
        ...
        const currentHistory = transactionHistory.find(
          (tx) => tx.id === txHash,
        );
        ...
        if (fetchedTransactions && fetchedTransactions.length > 0) {
          const completedTx = fetchedTransactions.find(
  ...
  const handleMigrationSuccess = (
    ...
    showToast(
      "Migration completed",
      "Your tokens have been successfully migrated to HPP network.",
      "success",
      {
      ...
      if (tokenTxData.status === "1") {
        ...
        const relevantTransactions = (tokenTxData.result || [])
          .filter((tx: any) => {
            ...
            return (
              tx.from?.toLowerCase() === walletLc &&
              tx.to?.toLowerCase() === contractLc
            );
          })
          .map((tx: any) => {
            ...
            return {
              id: tx.hash,
              type,
              date: dayjs(parseInt(tx.timeStamp) * 1000).format(
                "YYYY-MM-DD HH:mm:ss",
              ),
  ...
  const getTransactionIcon = (
    ...
    if (status === "Pending") {
      return (
        <PendingIcon className="w-8.5 h-8.5 text-white animate-spin [animation-duration:1.25s]" />
      );
    ...
    if (balance) {
      try {
        ...
        if (amountBig.gt(balanceBig)) {
          setInputError(
            `Insufficient balance. You have ${balanceBig.toFixed()} ${token}`,
          );
          setToAmount("");
          return;
        }
        ...
        if (numericValue > parseFloat(balance.replace(/,/g, ""))) {
          setInputError(
            `Insufficient balance. You have ${parseFloat(balance.replace(/,/g, "")).toFixed(4)} ${token}`,
          );
          setToAmount("");
          return;
        }
  ...
  const isMigrationDisabled = () => {
    ...
    if (balance && fromAmount) {
      try {
        ...
        // If Big.js fails, fallback to regular comparison
        if (parseFloat(fromAmount) > parseFloat(balance.replace(/,/g, "")))
          return true;
      }
    }
  ...
  const handleMigrationClick = async () => {
    ...
    try {
      ...
      if (allowanceData && allowanceData >= amountInWei) {
        ...
        showToast(
          "Approving...",
          "Please wait while we approve the transaction...",
          "loading",
        );
      ...
      showToast(
        "Error",
        error.message || "Failed to process migration",
        "error",
      );
  ...
  const handleSwapAergoForHpp = async () => {
    try {
      ...
      showToast(
        "Migrating...",
        "Starting HPP migration. Please wait...",
        "loading",
      );
      ...
      try {
        ...
        setTransactionHistory((prev) => [
          pendingTx,
          ...prev.filter((t) => t.id !== migrationHash),
        ]);
      } catch {}
      ...
      showToast(
        "Migration failed",
        error.message || "Failed to migrate tokens",
        "error",
      );
      setIsSwapping(false);
          ...
          <div className="py-7.5">
            <div className="px-5 max-w-6xl mx-auto">
              ...
              <p className="text-xl text-[#bfbfbf] font-semibold leading-[1.5] max-w-5xl text-center">
                Move your {token} tokens to the HPP Mainnet using the official
                migration paths.
                <br />
                Follow the steps carefully to ensure a secure and complete
                migration.
              </p>
                    ...
                    <div className="flex items-start mb-5">
                      <div className="flex-1">
                        ...
                        <p className="text-base font-normal text-white tracking-[0.8px] text-left leading-[1.5]">
                          If you hold AERGO on the AERGO mainnet, use the
                          official Aergo Bridge to migrate to HPP via Ethereum.
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="flex items-start mb-5">
                    <div className="flex-1">
                      ...
                      <p className="text-base font-normal text-white tracking-[0.8px] text-left leading-[1.5] whitespace-pre-wrap break-words">
                        If your {token} tokens are already on Ethereum, connect
                        your wallet and migrate directly.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="w-full h-min flex flex-row justify-center items-center p-5 overflow-hidden rounded-[5px]">
                    <div className="flex items-center gap-6 min-[480px]:gap-8 min-[640px]:gap-12 min-[810px]:gap-15">
                      ...
                      <div className="flex flex-col items-center">
                        <div className="w-10 h-10 min-[400px]:w-20 min-[400px]:h-20 min-[810px]:w-25 min-[810px]:h-25 min-[1440px]:w-27.5 min-[1440px]:h-27.5 rounded-lg flex items-center justify-center mb-2.5">
                          <Image
                            src={token === "AERGO" ? AergoEth : AqtEth}
                            alt={`${token}(ETH)`}
                          />
                        </div>
                        <span className="text-sm min-[400px]:text-base leading-[1.2em] tracking-[0.8px] font-normal text-white text-center -ml-1 min-[400px]:-ml-2.5">
                        ...
                        <div className="bg-white rounded-lg p-5 mb-4">
                          <div className="flex justify-end mb-2">
                            <button
                              ...
                              onClick={() => {
                                const cleanBalance = (balance || "0").replace(
                                  /,/g,
                                  "",
                                );
                ...
                <div className="mt-8">
                  <div className="bg-primary border border-[#161616] rounded-[5px] p-6">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-lg font-semibold leading-[1em] text-white">
                        Transaction History
                      </h3>
                      <button
                        onClick={() =>
                          address && fetchTransactionHistory(address)
                        }
                      ...
                      <div className="space-y-2">
                        {(showAllHistory
                          ...
                          <div
                            ...
                            style={{ cursor: "pointer" }}
                            onClick={() => {
                              const etherscanUrl = createEtherscanLink(
                                tx.hash,
                                tx.network,
                              );
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="hidden min-[600px]:flex items-center justify-between">
                                <div className="flex items-center space-x-3">
                                  <div>
                                    <div className="text-lg font-semibold text-white leading-[20px]">
                                      {tx.type}
                                    </div>
                              ...
                              <div className="min-[600px]:hidden space-y-1">
                                <div className="text-base font-semibold text-white leading-[20px]">
                                  {tx.type}
                                </div>
                ...
                <div className="p-6">
                  <div className="flex items-start mb-5">
                    <div className="flex-1">
                      ...
                      <p className="text-base font-normal text-white tracking-[0.8px] text-left leading-[1.5]">
                        Once you have HPP tokens on Ethereum, bridge them to the
                        HPP Mainnet for full ecosystem access(listings,
                        governance, utilities, and more).
                      </p>
                    </div>
                  </div>
